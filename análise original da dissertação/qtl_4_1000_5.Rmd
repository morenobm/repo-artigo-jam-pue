...
set.seed(22334455)
mimulus.sim <- sim.geno(mimulus.qtl.prob, n.draws = 550, step = 1, map.function = "kosambi")
#Escaneando o genoma em busca de QTL Haley-Knott
scan_result_hk <- scanone(mimulus.sim, method = "hk", pheno.col = mimulus.qtl$pheno$nmo)
summary(scan_result_hk)
plot(scan_result_hk)

# Calculando o limiar
set.seed(22334455)
operm.hk <- scanone(mimulus.sim, method="hk", n.perm=1000, pheno.col = mimulus.qtl$pheno$nmo)
summary(operm.hk)
...
flank_qtl_im <- find.flanking(mimulus.sim, 1, 5); flank_qtl_im
...
set.seed(22334455)
mimulus.sim <- sim.geno(mimulus.qtl.prob, n.draws = 550, step=1, map.function = "kosambi", stepwidth = "fixed")
...
summary(fit)
#Fazendo uma análise dos QTLs que tem maior efeito, dois deles não foram adicionados no modelo final por ter pvalue mais alto e R2 bem baixo, quase zero. Desse jeito eu vou testar de colocar apenas aqueles QTLs que tem maior valor e que provavelmente mudam coisas.

stepwise_qtl_2 <- data.frame ("name" = c("1@4.0", "2@76.2", "4.2@3.0", "7@133.0", "8@51", "9@47"),
                            "chr" = c(1,2,4.2,7,8, 9),
                            "pos" = c(4,76.2,3,133,51,47))
stepwise_qtls_2 <- makeqtl(mimulus.sim, chr = stepwise_qtl$chr, pos = stepwise_qtl$pos, qtl.name = stepwise_qtl$name, what = "prob")
fit <- fitqtl(mimulus.sim, pheno.col = mimulus.qtl$pheno$nmo, model = "normal", qtl = stepwise_qtls, method = "hk", get.ests = TRUE)
summary(fit)

stepwise_qtl <- data.frame ("name" = c("1@5.0", "2@75.0", "3.2@7.0", "4.2@3.0", "7@133.0", "8@80.3"),
                            "chr" = c(1,2,3.2,4.2,7,8),
                            "pos" = c(4,75,7,3,133,80.301))
stepwise_qtls <- makeqtl(mimulus.sim, chr = stepwise_qtl$chr, pos = stepwise_qtl$pos, qtl.name = stepwise_qtl$name, what = "prob")
fit_2 <- fitqtl(mimulus.sim, pheno.col = mimulus.qtl$pheno$nmo, model = "normal", qtl = stepwise_qtls, method = "imp", get.ests = TRUE)
summary(fit_2)
#Fazendo uma análise dos QTLs que tem maior efeito, dois deles não foram adicionados no modelo final por ter pvalue mais alto e R2 bem baixo, quase zero. Desse jeito eu vou testar de colocar apenas aqueles QTLs que tem maior valor e que provavelmente mudam coisas.
#################################3
##testes feitos depois de escrever que podem estar certos ou não!!
teste_apagar_depois <- data.frame ("name" = c("1@5.0", "2@11.0", "2@79.9", "2@95.0", "4.2@3.3", "7@130.0", "9@58.0", "10@0.0"),
                            "chr" = c(1,2,2,2,4.2,7,9,10),
                            "pos" = c(5,11,79.9,95,3.3,130,58,0))

teste_apagar_qtls <- makeqtl(mimulus.sim, chr = teste_apagar_depois$chr, pos = teste_apagar_depois$pos, qtl.name = teste_apagar_depois$name, what = "prob")
fit_2 <- fitqtl(mimulus.sim, pheno.col = mimulus.qtl$pheno$nmo, model = "normal", qtl = teste_apagar_qtls, method = "imp", get.ests = TRUE)
summary(fit_2)
...
##########################################
#CIM
cim_result <- cim(mimulus.sim, pheno.col= mimulus.qtl$pheno$nmo, n.marcovar=6, method="hk", window = 10, map.function = "kosambi")
#Setando o limiar
threshold.cim <- cim(mimulus.sim, pheno.col= mimulus.qtl$pheno$nmo, n.marcovar=6, method="hk", window = 10, n.perm = 1000, map.function = "kosambi")
...
refined <- refineqtl(mimulus.sim, pheno.col = mimulus.qtl$pheno$nmo, qtl = qtl_data, method = "hk", model = "normal")
summary(refined)
...
#refinando os qtls
qtl_ref <- refineqtl(mimulus.sim, pheno.col = mimulus.qtl$pheno$nmo, qtl = stepwise_qtls_2, method = "hk", model = "normal")
summary(qtl_ref)
#Marcadores que estão nos picos dos QTLs mapeados
...
for (i in 1:length(picos)) {
  ...
  efeitos_gen[[i]] <- effectplot(mimulus.sim, mname1 = picos[i], draw=TRUE, pheno.col = mimulus.qtl$pheno$nmo)
  ...
  efeitos_letras[[i]] <- plotPXG(mimulus.sim, marker = picos[i], pheno.col = mimulus.qtl$pheno$nmo, ylab = "Número de massas de ovos")
